...
**v0.2.8** (버전은 `config.js`의 `meta.version` 및 `CHANGELOG.md`와 항상 일치시킬 것)
...
(v0.2.8~ 화면은 **카드뉴스형 카드**(반투명 화이트/블루). 카드 상단 헤더에 좌측 "처음으로"
미니 버튼 + 중앙 STEP/MISSION 배지, 카드 하단에 페이지 점. 좌측 고정 컨트롤은 운영자용.)
